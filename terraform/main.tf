...
resource "kubernetes_deployment" "api" {
 metadata { name="ai-workflow-engine" namespace=var.namespace labels={app="ai-workflow-engine"} }
 spec { replicas=var.replicas selector {match_labels={app="ai-workflow-engine"}} template {metadata {labels={app="ai-workflow-engine"}} spec {container {name="api" image=var.image port {container_port=8000} resources {requests={cpu="100m",memory="128Mi"} limits={cpu="500m",memory="512Mi"}} readiness_probe {http_get {path="/health/ready" port=8000}} liveness_probe {http_get {path="/health/live" port=8000}}}}}}
}
resource "kubernetes_service" "api" { metadata {name="ai-workflow-engine" namespace=var.namespace} spec {selector={app="ai-workflow-engine"} port {port=80 target_port=8000}}}
